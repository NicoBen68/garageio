      ...
      <ScrollView contentContainerStyle={styles.inner} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.header}>
          ...
          <TouchableOpacity onPress={() => editing ? handleSave() : setEditing(true)}>
            {saving ? <ActivityIndicator color="#3B82F6" /> : <Text style={styles.editBtn}>{editing ? 'Sauvegarder' : 'Modifier'}</Text>}
          </TouchableOpacity>
        </View>

        <View style={styles.titleBlock}>
          <Text style={[styles.vehicleTitle, { color: c.textPrimary }]}>{brand} {model}</Text>
        ...
        <View style={[styles.mileageCard, { backgroundColor: c.card, borderColor: c.cardBorder }]}>
          <Text style={[styles.mileageLabel, { color: c.textSecondary }]}>🛣️ Kilométrage actuel</Text>
          <View style={styles.mileageRow}>
            <TextInput style={[styles.mileageInput, { backgroundColor: c.bg, borderColor: c.inputBorder, color: c.textPrimary }]} keyboardType="numeric" value={mileage} onChangeText={setMileage} />
            <Text style={[styles.mileageUnit, { color: c.textMuted }]}>km</Text>
          </View>
          {!editing && (
            <TouchableOpacity style={styles.mileageSaveBtn} onPress={async () => { await supabase.from('vehicles').update({ current_mileage: parseInt(mileage) || 0 }).eq('id', id); Alert.alert('✅ Kilométrage mis à jour !'); }}>
        ...
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: c.textMuted }]}>Informations</Text>
          <Text style={[styles.label, { color: c.textSecondary }]}>Marque</Text>
          <TextInput style={[styles.input, { backgroundColor: c.input, borderColor: c.inputBorder, color: c.textPrimary }, !editing && styles.inputDisabled]} value={brand} onChangeText={setBrand} editable={editing} />
          <Text style={[styles.label, { color: c.textSecondary }]}>Modèle</Text>
          <TextInput style={[styles.input, { backgroundColor: c.input, borderColor: c.inputBorder, color: c.textPrimary }, !editing && styles.inputDisabled]} value={model} onChangeText={setModel} editable={editing} />
          <View style={styles.row}>
            <View style={styles.halfField}>
              <Text style={[styles.label, { color: c.textSecondary }]}>Année</Text>
              <TextInput style={[styles.input, { backgroundColor: c.input, borderColor: c.inputBorder, color: c.textPrimary }, !editing && styles.inputDisabled]} keyboardType="numeric" value={year} onChangeText={setYear} editable={editing} />
            </View>
            <View style={styles.halfField}>
              <Text style={[styles.label, { color: c.textSecondary }]}>Couleur</Text>
              <TextInput style={[styles.input, { backgroundColor: c.input, borderColor: c.inputBorder, color: c.textPrimary }, !editing && styles.inputDisabled]} value={color} onChangeText={setColor} editable={editing} />
            </View>
          </View>
        </View>
        ...
        {editing ? (
          <View style={styles.section}>
            <Text style={[styles.sectionTitle, { color: c.textMuted }]}>Carburant</Text>
            <View style={styles.fuelGrid}>
              {FUEL_TYPES.map((f) => (
                <TouchableOpacity key={f} style={[styles.fuelBtn, { backgroundColor: c.card, borderColor: c.cardBorder }, fuelType === f && styles.fuelBtnActive]} onPress={() => setFuelType(f)}>
                  <Text style={[styles.fuelBtnText, { color: c.textSecondary }, fuelType === f && styles.fuelBtnTextActive]}>{FUEL_EMOJI[f]} {f.charAt(0).toUpperCase() + f.slice(1)}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </View>
          ...
          <View style={[styles.infoRow, { backgroundColor: c.card, borderColor: c.cardBorder }]}>
            <Text style={[styles.infoLabel, { color: c.textMuted }]}>Carburant</Text>
            <Text style={[styles.infoValue, { color: c.textPrimary }]}>{FUEL_EMOJI[fuelType]} {fuelType}</Text>
          </View>
        )}

        <TouchableOpacity style={[styles.maintenanceBtn, { backgroundColor: c.card, borderColor: c.cardBorder }]} onPress={() => router.push(`/(tabs)/vehicle/${id}/maintenance`)}>
          <Text style={[styles.maintenanceBtnText, { color: c.textPrimary }]}>🔧 Voir le carnet d'entretien</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  maintenanceBtn:     { borderRadius: 12, paddingVertical: 14, alignItems: 'center', borderWidth: 1 },
