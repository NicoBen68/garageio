...
function getUrgentReminder(reminders: Reminder[]): Reminder | null {
  ...
  // Trie par urgence : overdue d'abord, puis soon
    ...
    <FlatList
      ...
      ListHeaderComponent={() => (
        <View>
          ...
          {urgentReminder && (
            <TouchableOpacity
              style={[styles.alertCard, getUrgency(urgentReminder) === 'overdue' ? styles.alertCardOverdue : styles.alertCardSoon]}
              onPress={() => router.push('/(tabs)/reminders')}
            >
              <View style={styles.alertLeft}>
                <Text style={styles.alertEmoji}>
                  {getUrgency(urgentReminder) === 'overdue' ? '⚠️' : '🔔'}
                </Text>
                <View>
          ...
          {vehicles.length > 0 && (
            <View style={styles.statsRow}>
              <TouchableOpacity style={styles.statCard} onPress={() => router.push('/(tabs)/maintenance')}>
                <Text style={styles.statEmoji}>🔧</Text>
                <Text style={styles.statLabel}>Entretien</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.statCard} onPress={() => router.push('/(tabs)/reminders')}>
                <Text style={styles.statEmoji}>🔔</Text>
                <Text style={styles.statLabel}>
                  {reminders.length > 0 ? `${reminders.length} rappel${reminders.length > 1 ? 's' : ''}` : 'Rappels'}
                </Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.statCard} onPress={() => router.push('/(tabs)/add-vehicle')}>
                <Text style={styles.statEmoji}>＋</Text>
      ...
      ListEmptyComponent={() => (
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptySubtitle}>Saisis ta plaque et on s'occupe du reste.</Text>
...
const styles = StyleSheet.create({
  ...
  alertCardOverdue:{ backgroundColor: '#1A0F0F', borderColor: '#EF4444' },
  alertCardSoon:   { backgroundColor: '#1A160A', borderColor: '#F59E0B' },
  ...
  alertMore:       { fontSize: 11, color: '#64748B', marginTop: 2 },
  alertArrow:      { fontSize: 20, color: '#64748B' },
  ...
  sectionTitle:    { fontSize: 13, fontWeight: '600', color: '#64748B', textTransform: 'uppercase', letterSpacing: 0.5, paddingHorizontal: 24, marginBottom: 10 },
  ...
  card:            { backgroundColor: '#1E293B', borderRadius: 16, padding: 16, borderWidth: 1, borderColor: '#334155', gap: 12, marginHorizontal: 24, marginBottom: 12 },
